...
kline_btceur_1m = Kline("BTCEUR", interval="1m")
klines1 = kline_btceur_1m.get_historical(start_time="2022-11-20T19:30:00")
klines2 = kline_btceur_1m.get_historical(start_time="2022-11-20T19:30:00")
####On choisit le nombre de fois qu'on duplique les 1000 observations
####Exemple : si nb_dup = 2, le nombre totale d'observations est 1000*nb_dup=2000
nb_dup = 5
####stocker les données dans un dataframe afin de les stocker dans une base sql
start_time = time.time()
df_1000 = pd.DataFrame(columns=['Time', 'Open', 'High', 'Low', 'Close', 'Volume'])
i = 0
for kline in klines1:
    df_1000.loc[i, 'Time'] = datetime.fromtimestamp(kline[0])
    df_1000.loc[i, 'Open'] = kline[1]
    df_1000.loc[i, 'High'] = kline[2]
    df_1000.loc[i, 'Low'] = kline[3]
    df_1000.loc[i, 'Close'] = kline[4]
    df_1000.loc[i, 'Volume'] = kline[5]
    i += 1
df = pd.DataFrame()
for i in range(nb_dup):
    df = pd.concat([df, df_1000], ignore_index = True)
...
for kline in klines2:
    list_documents_1000.append({
        "Time" : datetime.fromtimestamp(kline[0]), 
        "Open" : kline[1], 
        "High" : kline[2], 
        "Low" : kline[3], 
        "Close" : kline[4], 
...
list_documents = []
for i in range(nb_dup):
    list_documents.extend(list_documents_1000)
