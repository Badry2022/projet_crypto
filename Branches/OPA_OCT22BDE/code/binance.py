...
import re
import numbers
from websocket_threaded import WebsocketThreaded
...
class Kline:
    ...
    def get_historical(self, start_time=KLINE_FIRST_DATETIME, exclude_start=False, end_time=None, exclude_end=False):
        ...
        timestamp = datetime.strptime(clean_time(start_time, interval), "%Y-%m-%dT%H:%M:%S")
        ...
        if end_time is not None:
            ...
            end_timestamp = datetime.strptime(clean_time(end_time, interval), "%Y-%m-%dT%H:%M:%S")
    ...
    def get_recent_data(self, last_date):
        frequency = self._interval
        current_date = datetime.now()

        delta = timedelta(days = 1)
        if frequency == KLINE_INTERVAL_1HOUR:
            delta = timedelta(hours = 1)
        elif frequency == KLINE_INTERVAL_1MINUTE:
            delta = timedelta(minutes = 1)
        
        next_expected_date = last_date + delta

        if (next_expected_date > current_date):
            print("##############Dbase is up to date###############")
            return
        else:
            next_expected_date = next_expected_date.strftime('%Y-%m-%dT%H:%M:%S')
            #print(next_expected_date)
            klines_data = self.get_historical(start_time = next_expected_date)
